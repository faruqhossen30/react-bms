...
const Modalbetnow = ({ matchData, questionData, optionData, isOpen, setIsOpen }) => {
    ...
    const onSubmit = data => {
        console.log(data)
        axios.post(`${process.env.REACT_APP_BASE_URL}/user/bet`,
            data
        )
            .then((res) => {
                console.log(res);
                        ...
                        <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                            <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                                <form onSubmit={handleSubmit(onSubmit)} >
                                    ...
                                    <input type="hidden" name="match_title" {...register("match_title")} defaultValue={optionData.bet_rate} />
                                    <input type="hidden" name="question_title" {...register("question_title")} defaultValue={optionData.bet_rate} />
                                    <input type="hidden" name="option_title" {...register("option_title")} defaultValue={optionData.bet_rate} />
                                    ...
                                    <div className="mt-2">
                                        ...
                                        <input type="number" name='bet_amount'  {...register("bet_amount", { required: 'bet_amount is requred.' })} id="name" placeholder="500" className="w-full p-1 border focus:border-purple-600" onChange={betAmountChange} />
                                        <span className='text-red-600 text-sm'>{errors.bet_amount?.message}</span>
                                    </div>
                                    <div>
